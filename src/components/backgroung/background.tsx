...
function NebulaBackground() {
  ...
  if (!("fov" in perspectiveCamera)) {
    // This is a fallback if camera is not perspective
    console.warn("Camera is not a PerspectiveCamera");
    return null; // or fallback rendering
  }

  const d = 900; // distance from camera
  const imgWidth = 1024;
...
export default function SpaceBackground() {
  ...
  return (
    ...
    >
      ...
      {/* <div
        className="absolute top-10 left-1/2 transform -translate-x-1/2 max-w-xl rounded-xl p-6 text-white shadow-lg border border-[#7f5af0]"
        style={{
          backgroundColor: "rgba(15, 20, 40, 0.6)",
          backdropFilter: "blur(10px)",
          borderImage: "linear-gradient(45deg, #7f5af0, #00f0ff) 1",
          borderImageSlice: 1,
        }}
      >
        <h1 className="text-3xl font-bold mb-4 text-[#00f0ff]">
          Explore the Cosmos
        </h1>
        <p className="text-lg leading-relaxed text-[#e5e7eb]">
          Move your cursor around to see gravitational lensing — light bending
          around a massive cosmic body, distorting the stars.
        </p>
      </div> */}
    </div>
  );
